...
export let getCommentAboveThatShouldExist = <T extends SortingNode>({
  ...
  let numberOfMatchingCommentsBetween = getCommentsBefore({
    node: sortingNode.node,
    sourceCode,
  }).find(comment => comment.value.trim() === groupAboveRight.commentAbove)
  return {
    exists: !!numberOfMatchingCommentsBetween,
