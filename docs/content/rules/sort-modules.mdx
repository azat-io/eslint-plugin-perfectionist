...
<Important>
  This option only has an impact with [`useExperimentalDependencyDetection: false`](#useexperimentaldependencydetection).
</Important>
...
When set, the rule reads `emitDecoratorMetadata` from your tsconfig and treats decorator-metadata type references as dependencies.
...
Specifies additional node types to analyze as module blocks, in addition to the built-in ones.

Each node type should have a `body` property containing an array of statements.

For example, `svelte-eslint-parser` wraps the content of a `<script>` block in a `SvelteScriptElement` node:

```ts
{
  additionalModuleBlockTypes: ['SvelteScriptElement']
}
```
